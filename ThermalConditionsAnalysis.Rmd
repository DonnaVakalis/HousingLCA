...
table(dat.surv.all$Pre_Winter,dat.surv.all$murb) %>%
    prop.table(margin=1) %>%
    round(3) %>%
     `*`(100)

table(dat.surv.all$Post_Winter,dat.surv.all$murb) %>%
    prop.table(margin=1) %>%
    round(3) %>%
     `*`(100)
...
# Make a dataframe with a list of locations, including their murb, that were in the set of sensor data 
...
temp.summer <-
    ...
    #select(tmp_out) %>%
    summarise(Average = mean(tmp_out, na.rm = T))
        
    ))# defined as average between June 21 and Sept 21
temp.winter # defined as average between Dec 21 and Feb 21
...
# mini sensitivity-analysis: check other temperatures
...
rm(dat.predicted.comf)
class(coef(m1c.lmer[[1]]))
class(dat.predicted.comf[])

head(dat.predicted.comf[1])

dat.predicted.com <- reduce(full_join, dat.predicted.comf[])   

dat.predicted.comf %>% reduce(full_join)
