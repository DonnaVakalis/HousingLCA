...
```{r}
##### REGRESSION MODEL MIXED EFFECTS e.g., RANDOM INTERCEPT  ============
...
typeof(dat.Sensor.Weather$com_upper)
...
#####  MODEL COMPARISON AND EVALUATION + BOX PLOT MEANS? ============
...
# VISUALIZE
(mm_plot <- ggplot(dat.mr[[7]], aes(x = tmp_out, y = com_upper, colour = retrofit_yes)) +
      facet_wrap(~locID, nrow=2) +   # a panel for each suite
      geom_point(alpha = 0.2) +
      theme_classic() +
      geom_line(data = cbind(dat.mr[[7]], pred = predict(m1c.lmer[[7]])), aes(y = pred), size = 1) +  # adding predicted line from mixed model 
