...
dat.surv.pre <-
     file.surv.pre[c(1,4:7,53:64)] #Keep ResponseID, building, suite ID, and answers to thermal comfort questions
dat.surv.post <- 
    file.surv.post[c(9:10,15,61:72)] %>% 
    rename (ResponseID = occupant_import_dat_ResponseID) %>%
    mutate (First_survey = 0)
    
 
 
dat.surv.all <-
    merge(dat.surv.pre , dat.surv.post, all.x=TRUE, all.y = TRUE)

 
 
Check_Respondants_common<-
  dat.surv.all %>%
  group_by(First_Survey) %>%
