...
# check average Feb (Winter) temperatures pre and post survey
df.ave.tmp <-
    dat.weather %>%
    mutate(survey.season = 
             case_when( date %within% interval(ymd("15-01-01"),ymd("15-02-28") ) ~ "pre.wntr",
                        date %within% interval(ymd("18-01-01"),ymd("18-02-28") ) ~ "post.wntr",
                        date %within% interval(ymd("14-07-01"),ymd("14-08-28") ) ~ "pre.smmr",
                        date %within% interval(ymd("17-07-01"),ymd("17-08-28") ) ~ "post.smmr")) %>%
  
    group_by(survey.season)%>%
    summarize(ave.tmp = mean(tmp_out))
...
(mm_plot <- ggplot(dat.mr[[ii]], aes(x = tmp_out, y = com_upper, colour = retrofit_yes)) +
      facet_wrap(~locID, nrow=3) +   # a panel for each suite
...
ggsave(
        filename= "modelled_comfort_6.pdf",
        plot = last_plot(),
        device = NULL,
...
# ADDITIONAL MODEL, ONLY LOWEST AND HIGHEST OUTDOOR TEMPERATURES ============

# Make new variable "tmp_cat"
# One each for tertile of outdoor temp
for (i in 1:7) {
    dat.mr[[i]]$tmp_cat <- 
        as.numeric(cut_number(dat.mr[[i]]$tmp_out,3))
}

  

# Model COLDEST/HOTTEST outdoor temps
# MODEL X: SAME AS 1B, but restrict to coldest OR hotest days (one model for each of those)

mx.lmer<- list() #Create a list in which I will save all models (one per building) of one type (e.g., mixed effect, logit)
mx.summary <- list()#Create a list for each summary of model
for (i in 1:7) {
    input <- dat.mr[[i]] %>%
        subset(tmp_cat==3) #select outdoor temp tertile
    model1<- lmer(com_upper ~ poly(tmp_out,2) + retrofit_yes + (1|locID),
                 REML=FALSE,
                 data=input)
    mx.lmer[[i]] <- model1 
    mx.summary[[i]]<-summary(model1)
}
...
# SUMMARIZE AND VISUALIZE RESULTS OF MODEL X ============
ii=7 # MURB ID
...
bg_data <- 
    dat.mr[[ii]] %>% 
        subset(tmp_cat==3) # set outdoor temp tertile
  
(mm_plot <- ggplot(bg_data, aes(x = tmp_out, y = com_upper, colour = retrofit_yes)) +
      facet_wrap(~locID, nrow=2) +   # a panel for each suite
      geom_point(alpha = 0.2) +
      theme_classic() +
      geom_line(data = cbind(bg_data, pred = predict(mx.lmer[[ii]])), aes(y = pred), size = 1) +  # adding predicted line from mixed model 
      theme(legend.position = "bottom") +   
      scale_color_manual(values = c(colour2, colour1))+
      ylim(0,1)
)
...
plot.S<-
    dat.surv.all %>%
    dplyr::select(murb,Pre_Summer,Post_Summer) %>%
...
plot.W<-
    dat.surv.all %>%
    dplyr::select(murb,Pre_Winter,Post_Winter) %>%
...
p<- ggarrange(Heat.map.summer, Heat.map.winter, ncol=2, nrow=1, common.legend = TRUE, legend="right")
#annotate_figure(p, top=text_grob("How Comfortable are you in different rooms of your apartment?"))
ggsave(path="plots_doNotUpload", filename="results_4.pdf", width = 10, height = 4) 
rm(p)
...
# Pre-retrofit respones from those who dropped out
pop.lost <- 
    dat.surv.all %>%
    filter(First_survey == TRUE) %>%
    filter(is.na(Second_survey)) %>%
    select(murb,Pre_Summer,Pre_Winter)%>%
    mutate(Stayed = TRUE)

# For comparison, pre-retrofit responses from those who did not drop out
pop.stayed <-
    dat.surv.all %>%
    filter(First_survey == TRUE) %>%
    filter(Second_survey == TRUE) %>%
...
dat.followup <-
    rbind(pop.stayed, pop.lost) 

dat.followup$Pre_Summer <- ordered(dat.followup$Pre_Summer, levels = c("Too cold","Just right","Too warm","Don't know"))
dat.followup$Pre_Winter <- ordered(dat.followup$Pre_Winter, levels = c("Too cold","Just right","Too warm","Don't know"))

#check.count_responses
#sapply(pop.lost,class) 
#sapply(pop.lost,levels)
#xtabs( ~ murb + Pre_Summer, data = pop.stayed)


# VISUALIZE the Proportions in each group (stay versus lost)
xtabs(~Stayed + Pre_Winter,
       data = dat.followup)

library(lattice)

histogram(~ Pre_Winter| Stayed,
          data=dat.followup,
          layout=c(1,2)      #  columns and rows of individual plots
          )

# perform Mann–Whitney U test for each Summer/Winter


# first need to assign the survey responses to ordinals (1,2,3,4)
levels(dat.followup$Pre_Summer)[levels(dat.followup$Pre_Summer)=="Too cold"] = as.numeric(1)
levels(dat.followup$Pre_Summer)[levels(dat.followup$Pre_Summer)=="Just right"] = as.numeric(2)
levels(dat.followup$Pre_Summer)[levels(dat.followup$Pre_Summer)=="Too warm"] = as.numeric(3)
levels(dat.followup$Pre_Summer)[levels(dat.followup$Pre_Summer)=="Don't know"] = as.numeric(4)
dat.followup$Pre_Summer = as.numeric(dat.followup$Pre_Summer)

levels(dat.followup$Pre_Winter)[levels(dat.followup$Pre_Winter)=="Too cold"] = as.numeric(1)
levels(dat.followup$Pre_Winter)[levels(dat.followup$Pre_Winter)=="Just right"] = as.numeric(2)
levels(dat.followup$Pre_Winter)[levels(dat.followup$Pre_Winter)=="Too warm"] = as.numeric(3)
levels(dat.followup$Pre_Winter)[levels(dat.followup$Pre_Winter)=="Don't know"] = as.numeric(4)
dat.followup$Pre_Winter = as.numeric(dat.followup$Pre_Winter)

xtabs(~Stayed + Pre_Winter,
       data = dat.followup)

sapply(dat.followup,class) 

# Now perform MW U test
m1<-wilcox.test(Pre_Summer ~ Stayed, data=dat.followup, exact=FALSE, conf.int=TRUE)
print(m1)
# p_value = 0.09146 Can't reject the Null hypothesis that the distributions of both groups are identical, 

m2<-wilcox.test(Pre_Winter~ Stayed, data= dat.followup, exact=FALSE, conf.int=TRUE)
print(m2)           
#p_ value = 0.04062 
