...
dat.GWP.savings <- #new rows to add GWP of annual natural gas use delta, enter as: SiteID, retrofit.action=ALL, variable, amount
    c("1",as.factor("ALL"),as.factor("GWP.savings"),1234)
    
iris$Species <- factor(iris$Species, levels = c(levels(iris$Species), "Echeveria"))

dat.for.plot <-
    rbind(dat.for.plot,dat.GWP.savings)
    

# Visualize
    ggplot(dat.for.plot, aes(x = variable,
                         y=Amount,  
