...
dat.averages<-
    dat.for.plot %>%
    mutate(temp.col=4) %>%
    mutate(siteID = recode_factor(temp.col,
                                  "4"="4",
                                  "3"="3",
                                  "2"="2",
                                  "1"="1"))

setattr(dat.averages$siteID,"levels",c(1,2,3,4))
str(dat.averages)
    dat.averages$SiteID <-  # need to explicitly make this into a factor with levels that will match next merge (1,2,3,4)
        factor(dat.averages$SiteID) 
dat.averages<-  
...
# example  mutate(Sector = recode_factor(sector,
...
dat.for.plot<-
    merge(dat.averages,dat.for.plot, all=TRUE) %>% 
    melt(
